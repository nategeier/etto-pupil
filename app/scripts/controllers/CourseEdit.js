...
angular.module("ettoPupil")
  .controller("CourseEditCtrl", ["$scope", "course", "CoursePlayer", "$rootScope", "Tiers", "$state", "$stateParams",
    function ($scope, course, CoursePlayer, $rootScope, Tiers, $state, $stateParams) {
      ...
      var updatedCourse = "courseUpdated" + String($scope.course._id);

      socket.on(updatedCourse, function (data) {
      ...
      $scope.$on("course-save", function () {
        ...
        delete emmitedCourse.$promise;
        delete emmitedCourse.$resolved;

        socket.emit("updatedCourse", emmitedCourse);
